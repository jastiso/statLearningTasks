...
jsPsych6.plugins.instructions = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      pages: {
        type: jsPsych6.plugins.parameterType.HTML_STRING,
        pretty_name: 'Pages',
        default: undefined,
        array: true,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function btnListener(evt){
    	...
    	if(this.id === "jspsych6-instructions-back"){
    		back();
    	}
    	else if(this.id === 'jspsych6-instructions-next'){
    		next();
    	}
    }
    ...
    function show_current_page() {
      ...
      if (trial.show_clickable_nav) {
        ...
        var nav_html = "<div class='jspsych6-instructions-nav' style='padding: 10px 0px;'>";
        if (trial.allow_backward) {
          let allowed = (current_page > 0  && trial.show_page_number)? '' : "disabled='disabled'";
          nav_html += "<button id='jspsych6-instructions-back' class='jspsych6-btn' style='margin-right: 5px;' "+allowed+">&lt; "+trial.button_label_previous+"</button>";
        }
        if (trial.pages.length > 1 && trial.show_page_number) {
            nav_html += "<span style='margin: 0 1em;' class='"+
                "jspsych6-instructions-pagenum'>"+pagenum_display+"</span>";
        }
        nav_html += "<button id='jspsych6-instructions-next' class='jspsych6-btn'"+
        ...
        if (current_page != 0 && trial.allow_backward) {
          display_element.querySelector('#jspsych6-instructions-back').addEventListener('click', btnListener);
        }

        display_element.querySelector('#jspsych6-instructions-next').addEventListener('click', btnListener);
      } else if (trial.show_page_number && trial.pages.length > 1) {
          // page numbers for non-mouse navigation
          display_element.innerHTML += "<div class='jspsych6-instructions-pagenum'>"+
            pagenum_display+"</div>"
      }
    }
    ...
    function endTrial() {
      ...
      if (trial.allow_keys) {
        jsPsych6.pluginAPI.cancelKeyboardResponse(keyboard_listener);
      }
    ...
    var after_response = function(info) {
      ...
      keyboard_listener = jsPsych6.pluginAPI.getKeyboardResponse({
      ...
      if (jsPsych6.pluginAPI.compareKeys(info.key, trial.key_backward)) {
        if (current_page !== 0 && trial.allow_backward) {
          back();
        }
      }

      if (jsPsych6.pluginAPI.compareKeys(info.key, trial.key_forward)) {
        next();
      }
    ...
    if (trial.allow_keys) {
      var keyboard_listener = jsPsych6.pluginAPI.getKeyboardResponse({
        callback_function: after_response,
        valid_responses: [trial.key_forward, trial.key_backward],
        rt_method: 'date',
        persist: false
