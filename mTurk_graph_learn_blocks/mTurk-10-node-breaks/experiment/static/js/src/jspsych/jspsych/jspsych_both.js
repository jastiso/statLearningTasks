/**
 * jspsych.js, contains documentation from jsPsych 5.0 and jspsych 6.2 (labeled jspsych6)
 * Josh de Leeuw
 *
...
window.jsPsych6 = (function() {
  ...
  core.init = function(options) {
    ...
    checkExclusions(opts.exclusions,
      function(){
        ...
        if(opts.auto_preload){
          jsPsych6.pluginAPI.autoPreload(timeline, startExperiment, opts.preload_images, opts.preload_audio, opts.show_preload_progress_bar);
  ...
  function setDefaultValues(trial){
    var trial_parameters = Object.keys(jsPsych6.plugins[trial.type].info.parameters);
    for(var i=0; i<trial_parameters.length; i++){
      if(typeof trial[trial_parameters[i]] == 'undefined' || trial[trial_parameters[i]] === null){
        if(typeof jsPsych6.plugins[trial.type].info.parameters[trial_parameters[i]].default == 'undefined'){
          console.error('You must specify a value for the '+trial_parameters[i]+' parameter in the '+trial.type+' plugin.');
        } else {
          trial[trial_parameters[i]] = jsPsych6.plugins[trial.type].info.parameters[trial_parameters[i]].default;
        }
      }
    }
  }
  ...
  function updateProgressBar() {
    ...
    document.querySelector('#jspsych6-progressbar-inner').style.width = progress.percent_complete + "%";
  }
...
jsPsych6.pluginAPI = (function() {
  ...
  module.autoPreload = function(timeline, callback, images, audio, progress_bar) {
    ...
    images = jsPsych6.utils.unique(images);
    audio  = jsPsych6.utils.unique(audio);
