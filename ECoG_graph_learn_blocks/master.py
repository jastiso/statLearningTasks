...
# get subjID
subjDlg = gui.Dlg(title="Key Press Study")
...
def fullTask():
    ...
    prac_logname,motor_pracTrials=set_trials(subj_id,sess)

    #instructions 
    print('Loop through instructions and quiz')
    type_resp=motor1.instructOnly()
    
    #practice
    print('Practice for PreTask')
    motor1.do_prac(motor_pracTrials, prac_logname)
    ...
    log_file,logname,motor_trials1 = motor2.set_WalkData(subj_id,sess)
    
    #Walk round 1
    acc=motor2.do_run(subj_id,motor_trials1,logname.replace('.csv','.csv'),1,type_resp,sess)
...
def Task_NoTrain():
    ...
    #set values and parameters for tasks
    print('Set Vals for Task')
    motor_trainTrials, motor_pracTrials=set_trials(subj_id,sess)
    ...
    #Walk round 1
    acc = motor2.do_run(subj_id,motor_trials1,logname.replace('.csv','_run1.csv'),1)
